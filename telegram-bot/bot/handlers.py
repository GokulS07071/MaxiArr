...
async def handle_media_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    rating = item.get("ratings", {}).get("value", "N/A") if isinstance(item.get("ratings"), dict) else item.get("ratings", "N/A")
...
async def handle_add_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    profile_msg = await context.bot.send_message(
        chat_id=query.message.chat_id,
        text="⏳ Fetching available quality profiles..."
    )
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not queue:
        await progress_message.edit_text("✅ No active downloads in queue.")
        return
    ...
    await progress_message.edit_text("\n".join(html_lines), parse_mode="HTML")
