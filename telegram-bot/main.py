...
def main() -> None:
    ...
    application = (
        Application.builder().token(TELEGRAM_BOT_TOKEN).post_init(post_init).post_shutdown(post_shutdown).build()
    )
    ...
    application.add_handler(CallbackQueryHandler(handlers.handle_refresh_status, pattern="^refresh_status$"))
